...
This formation mechanism results naturally in thin planes, but a follow up work shows that these are not kinematically coherent structures (Buck et al. 2015). These results are from dark matter only simulations, so it would be interesting to see if they still hold for simulations that include baryonic physics, since it was shown by Gonzales et al 2015 that satellites accreted via filaments are destroyed faster than those that are not. 
So far, the planes found in simulations tend to be transient structures (XXXXXXX) and that poses two problems: one is why do we
...
We then calculate the plane width of the best fit plane. %de pronto acá deberia explicar mejor el metodo de plane fitting

...
We use this approach on simulations to build an explicit probability
distribution of finding an aspherical  satellite  distribution around
galaxies similar to the Milky Way and M31. 
This probability distribution allows us to estimate the atipicallity
of the satellite distributions in the LG even when the underlying
simulations do not have enough systems that resemble the LG satellite
anisotropy. 
This methodology is straightforward to implement, allowing
for an straighforward quantification of aspherical satellite
distributions both in simulations and observations.
We demonstrate the method using three different simulations:
Illustris-1,  Illustris-1-DarkMatter and ELVIS simulations. 
We find as a common trend that at most of $0.20\%$ the pairs are
expected  to have satellite distributions with the same degree of
deviation from sphericity as the Local Group.
The three simulations show that the satellite distribution in M31 is
common while the very planar MW satellite distribution is hard to
find.
We show that in these simulations between $19\%$ to $24\%$ of the pairs
have a halo with a satellite distribution similar to M31, while only
$0.80\%$, at most, have a satellite distribution as planar as the MW.
This characteristic places the LG as at the level of a 3-$\sigma$
outlier in context provided by LCDM simulations.
\end{abstract}

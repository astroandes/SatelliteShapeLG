...
Illustris-1,  Illustris-1-DarkMatter and ELVIS. 
We find three common trends in these simulations. First, at most of
$1\%$ the pairs are expected to have satellite distributions with the
same degree of deviation from sphericity as the Local Group; second, between
...
The M31 galaxy turns out to have a rather typical distribution 
in the LCDM context, for its brightest satellites, while the MW is
...
matrices. This approach will facilitate the direct comparison of the
results from different numerical setups and choices to select LG pairs
in simulations. 
\end{abstract}
...
distribution of interest. Then, we measure the same quantities for the spherical
randomizations, to finally renormalize the initial results to the mean
value and standard deviation from the randomized samples. 

We do not look for planes  or coherent kinematical structures in the
interest of keeping our study simple and robust.
In observations we limit our analysis to the 11 brightest satellites 
in the Milky Way and Andromeda. 
For numerical data we use the Illustris-1, Illustris-1-DM and the
ELVIS simulations.  

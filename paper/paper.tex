...
\begin{abstract}
We use numerical simulations to build an explicit probability distribution for the
asphericity in the satellite distribution around galaxies similar to the
Local Group (LG) in the Lambda Cold Dark Matter (LCDM) paradigm.
...
The approach presented here will facilitate the comparison between
...
Figure \ref{fig:correlations_illustrisdm} illustrates the results from
 ...
 Eq.\ref{eq:multivariate} from the normalized quantities in the
 simulation.   
The distributions are computed from $10^6$ with the multivariate
gaussian. 

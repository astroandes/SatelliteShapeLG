...
(\cite{2013MNRAS.429..725S} and \cite{2016MNRAS.457.1931S}) report
...
these three reports. \citep{2015ApJ...815...19P,2016MNRAS.457.1931S}.
Meanwhile, \cite{2013MNRAS.429.1502W} and \cite{2014ApJ...789L..24P} reported
on new tests using high resolution simulations and cosmological
...
do not show any special feature.
All published studies agree on the point that the global satellite distribution of
the 15 to 27 brightest satellites are consistent with 
spherically symmetric distribution and easy to reproduce in LCDM
simulations \citep{2006AJ....131.1405K,
  2007MNRAS.374.1125M, 2013ApJ...766..120C}.    
Table \ref{table:M31} and Table \ref{table:MW} summarize some more
details on the results we have just mentioned.
...
Trying to reconcile and understand the seemly conflicting or inconclusive
results on the MW has its origin on the frequentist fashion generally used to
compute probabilities.
Usually, the process starts by building a high level parent sample in the
simulation and then counting how many elements has a subset
meeting some criteria. 
The first inconvenient with this approach is that the probability
estimate is made against whatever turns out to be typical in each
simulation. 
Comparing results across simulations would require first
characterizing the simulations at the high level parent samples.
The second inconvenient is that it is hard to find systems that fully resemble
the LG, this means that the high level parent sample has a small size
making it hard to find elements that fit some criteria. 


In this paper we surmount the first difficulty by setting as direct 
point of reference an spherical satellite distribution and not the
simulations themselves.
The spherical distribution is built from the data itself
...
We then characterize each satellite distribution in terms of the
scalars describing its deviation from the spherical distribution.
We handle the second difficulty by building an explicit analytic
probability distribution of the deviations from asphericity; this
...
compatible with simulations, overcoming the problem of having a small
number of systems in the parent simulations.

To summarize, we start by using asphericity to characterize in the
same way simulations and observations. 
Then, we build an explicit probability distribution for the asphericity and
use simulations to estimate the parameters in this
distribution. 

...
\begin{itemize}
\item {\bf Stellar mass}. LGA galaxy pairs are composed by galaxies with stellar mass in the
range $1\times10^{10}\Msun <M_{\star}<1.5 \times 10^{11} \Msun$.
\item {\bf Dominant pair}. For each galaxy $A$ we find its closest galaxy $B$, if galaxy $A$ is also
...
\item {\bf Separation}. The distance between the galaxies' center of
  mass must be in the range $500$ kpc $<d_{AB}<$ $1500$ kpc. 
\item {\bf Isolation}. For each galaxy in the pair there cannot be a galaxy more
  massive than the lightest galaxy in the pair within a radius of $2$
  Mpc. 
\end{itemize}

We find $19$ pairs of galaxies fulfilling these conditions.
...
$M_{\star}$ & $10^{10}$\Msun & Stellar mass\\
$N_s$ & & Number of satellites with $M_V<-9$\\
$a > b> c$ & & Inertia tensor eigenvalues. \\
$\hat{s}_1$, $\hat{s}_2$, $\hat{s}_3$ & & Inertia tensor eigenvectors. \\
$\beta$  &  & Satellite Velocity Anisotropy\\
$\sigma_s$ & kpc & Plane width\\
$\lambda_1>\lambda_2>\lambda_3$ &  & Hessian Matrix
eigenvalues\\
$\hat{e}_{1}$,  $\hat{e}_{2}$,  $\hat{e}_{3}$ & - & Hessian Matrix
eigenvectors\\ 
...
Following \cite{} we want to find the cosmic web preferential
direction towards which the vectors $\hat{r}_{AB}$, $\hat{s}_3$ and 

...
\begin{abstract}
Here we quantify the asphericity in the spatial distribution of the
brightest satellites around the Milky Way (MW) and M31, the dominant
galaxies in the Local Group (LG).
We build explicit probability distributions of finding an aspherical
satellite distribution. 
This allows us to estimate the atipicallity of the satellite
distributions in the LG even when the underlying simulations
do not have enough systems that resemble the LG features.
This is also standarized method to compare different simulations and
is also convenient for reproducibility and comparison 
We demonstrate the method using three different simulations:
Illustris-1,  Illustris-1-DarkMatter and ELVIS simulations.
Although there are significant differences between the three
simulations, we find that at most of $0.20\%$ the pairs are expected
to have satellite distributions with the same degree of deviation from
sphericity as the Local Group.
The three simulations show that the satellite
distribution in M31 is common while the very planar MW satellite
distribution is hard to find; we expect that between $19\%$ to $24\%$
of the pairs have halo similar to M31, while only $0.80\%$ are similar
to the MW.
...
that are consistent by construction with their parent
simulation. Finally, we use the generated samples to estimate how
commoon are the deviations from sphericity that we measure in the observational
data.  We use a single-tail test in this comparison, meaning that we
always measure the fraction of points with values smaller than the
treshold value. 
...
\includegraphics[width=0.30\textwidth]{scatter_ranked_width.pdf}
\includegraphics[width=0.30\textwidth]{scatter_norm_ranked_width.pdf}
...
\includegraphics[width=0.30\textwidth]{scatter_ranked_ca_ratio.pdf}
\includegraphics[width=0.30\textwidth]{scatter_norm_ranked_ca_ratio.pdf}
...
\includegraphics[width=0.30\textwidth]{scatter_ranked_ba_ratio.pdf}
\includegraphics[width=0.30\textwidth]{scatter_norm_ranked_ba_ratio.pdf}
\caption{Same layout as in Figure \ref{fig:scatter_width}. 
...
\begin{table*}
  ...
  \end{tabular}
  \caption{Mean values and standard deviations for the different
    quantities describing satellite distributions: plane width, $c/a$
    ratio and $b/a$ ratio. The first colum refers to the results from
    observational data, the second column uses the spherically
    randomized version of the observational data. 
    The third column summarizes the results from the 20 pairs in
    Illustris-1 and the last column corresponds to the 12 pairs in the
    ELVIS project.}
\end{table*}

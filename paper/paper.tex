...
an spherical satellite distribution and not the simulations themselves.
...
We make this choice to to rule out the influence of satellite numbers
in the statistics. 
...
satellites included in the analysis.
...
\begin{equation}
{\bf{\bar{I}}} = \sum_{k=1}^{N_p}[(\bf{r}_i - \bf{r}_0)^2\cdot \bf{1} -
...
distributions for the scalars describing aesphericity.
...
Although the physical quantities of interests are bound, unlike the
...
That is, out of the the $n$ pairs in each simulation, we perfom $n$
...
foricorresponding to all the simulations are listed in the Appendix \ref{appendix:covariance}.
...
We estimated the mean and covariance of this distributions using
...
We estimated that with the M31 between $37\%$ and $58\%$ of the pairs
pairs show aspherical characteristics larger than M31, while
this fraction drops to less than $1\%$ for the MW. 
These fractions are robust to changes in the numerical simulations, 
the criteria used to define the pairs and the methods used to
...
observables as its the case of interest, as it allows for an atypicallity
...
Clouds, only expected in $5\%$ of galaxies \citep{2011ApJ...743..117B}
and the atypical kinematics of M31 with a very low tangential velocity,
only expected in less than $1\%$ of the pairs with similar
environmental characteristics \citep{ForeroRomero2013}. 
In other words, if we were in a typical LG pair in LCDM, the MW should
have zero satellites like the Magellanic Clouds, it's satellite
distribution should be similar to M31's and M31 should have a larger
tangential velocity.

The LG atypicality should be seen as an opportunity to constrain in
high detail the environment that allowed such a pattern to emerge. 
Although broad correlations between LG assembly, pair kinematics, halo shapes and
satellite distributions are expected in LCDM
...
understand what fearures in the initial conditions of our LG

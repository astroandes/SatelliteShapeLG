...
\begin{abstract}
In this paper we quantify the joint spatial distribution of the brightest satellites
around the Milky Way (MW) and M31, the dominant galaxies in the Local
Group (LG).
We use the Illustris-1 and ELVIS simulations to quantify how the 
unusual satellite distribution in the MW  makes the LG an atypical pair.
A crucial aspect in our analysis is that we define the atypicallity as
the degree at which a satellite distribution deviates from sphericity and
not as a deviation from simulations.
We find that only $0.05\%$ to $0.25\%$ of the pairs with isolation
